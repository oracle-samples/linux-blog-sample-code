...
/* Test ingress redirect; change ICMP code to bogus value (1) and redirect;
 * next time we find this code, change it back (otherwise checksum validation
 * will fail etc).
 */
BPF_HELPER_TEST_FUNC(bpf_redirect, in, ingress)
{
	...
	case 0:
		...
		if (ret != TC_ACT_REDIRECT) {
			bpf_debug("bpf_redirect returned %d\n", ret);
...
BPF_HELPER_TEST_FUNC(bpf_redirect, in, egress)
{
	/* Nothing to do here. */
	return TC_ACT_OK;
}
 ...
 * since we just swap src/dst.
 */
BPF_HELPER_TEST_FUNC(bpf_redirect, out, ingress)
{
